param(
    [ValidateSet('focus', 'dedicated', 'dedicated-six', 'full-modal')]
    [string]$Mode = 'full-modal',
    [int]$MaxProcessSeconds = 100,
...
$config = @{
    'focus' = @{
        Fixture = $CcFixture
        Harness = 'profile_pro_focus_p09.py'
        Result = 'pro_09_focus.json'
    }
    'dedicated' = @{
        Fixture = $DedicatedFixture
        Harness = 'profile_pro_modal_p09.py'
        Result = 'pro_09_modal.json'
    }
    'dedicated-six' = @{
        Fixture = $DedicatedFixture
        Harness = 'profile_pro_focus_p09.py'
        Result = 'pro_09_dedicated.json'
    }
    'full-modal' = @{
        Fixture = $CcFixture
        Harness = 'profile_pro_full_modal_p09.py'
        Result = 'pro_09_full_modal.json'
    }
}[$Mode]
...
foreach ($path in @($Blender, $Fixture, $Harness)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "PERF-P09 required file missing: $path"
    }
}
...
if ($shaBefore -ne $shaCheck) {
    throw "PERF-P09 fixture SHA unstable before launch: $shaBefore / $shaCheck"
}
if ($Mode -in @('focus', 'full-modal') -and $shaBefore -ne $ExpectedCcSha) {
    throw "PERF-P09 cc.blend SHA mismatch: $shaBefore"
}
...
if ($Mode -eq 'dedicated-six') {
    $arguments += @('--mode', 'dedicated')
}
...
if ($shaAfter -ne $shaBefore) {
    throw "PERF-P09 fixture changed: before=$shaBefore after=$shaAfter"
}
if ($timedOut) {
...
if ($exitCode -ne 0) {
    throw "PERF-P09 Blender failed for $Mode with exit code $exitCode"
}
if (-not (Test-Path -LiteralPath $Result -PathType Leaf)) {
    throw "PERF-P09 result missing: $Result"
}
...
if ($evidence.fixture_sha256_before -ne $shaBefore -or
    ...
    throw 'PERF-P09 evidence SHA mismatch'
}
...
$summary = if ($Mode -eq 'full-modal') {
    'status={0}; ticks={1}; records={2}; pairs={3}; shape_ops={4}; shape_slices={5}; max_tick_ms={6}; max_shape_slice_ms={7}; max_shape_call_ms={8}' -f `
        $evidence.status,
        $evidence.timer_ticks,
        $evidence.session_report.planner_record_count,
        $evidence.session_report.candidate_pairs_processed,
        $evidence.session_report.shape_primitive_ops,
        $evidence.session_report.shape_slices,
        $evidence.session_report.max_tick_ms,
        $evidence.session_report.max_shape_slice_ms,
        $evidence.session_report.max_shape_call_ms
} elseif ($Mode -eq 'dedicated') {
    'status={0}; max_tick_ms={1}; max_shape_slice_ms={2}; max_shape_call_ms={3}' -f `
        $evidence.status,
        $evidence.max_tick_ms,
        $evidence.completion.result.max_shape_slice_ms,
        $evidence.completion.result.max_shape_call_ms
} elseif ($Mode -eq 'dedicated-six') {
    'status={0}; cases={1}; aligned={2}' -f `
        $evidence.status,
        $evidence.cases.Count,
        (($evidence.cases | ForEach-Object { $_.result.aligned_exact }) -join '/')
} else {
    'measured={0}; elapsed={1}' -f $evidence.measured_runs, (($evidence.measured_elapsed_ms -join '/'))
}
Write-Output ("PERF-P09 {0}: {1}; peak_ws={2}; SHA={3}" -f $Mode, $summary, $peakWorkingSet, $shaAfter)
