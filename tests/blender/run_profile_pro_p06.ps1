...
$config = @{
    'focus' = @{
        Fixture = $CcFixture
        Harness = 'profile_pro_focus_p06.py'
        Result = 'pro_06_focus.json'
        RequirePassed = $true
    }
    'dedicated' = @{
        Fixture = $DedicatedFixture
        Harness = 'profile_pro_modal_p06.py'
        Result = 'pro_06_modal.json'
        RequirePassed = $true
    }
    'full-modal' = @{
        Fixture = $CcFixture
        Harness = 'profile_pro_full_modal_p06.py'
        Result = 'pro_06_full_modal.json'
        RequirePassed = $true
    }
}[$Mode]
...
foreach ($path in @($Blender, $Fixture, $Harness)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "PERF-P06 required file missing: $path"
    }
}
...
if ($shaBefore -ne $shaCheck) {
    throw "PERF-P06 fixture SHA was unstable before launch: $shaBefore / $shaCheck"
}
if ($Mode -in @('focus', 'full-modal') -and $shaBefore -ne $ExpectedCcSha) {
    throw "PERF-P06 cc.blend SHA mismatch: $shaBefore"
}
...
if ($shaAfter -ne $shaBefore) {
    throw "PERF-P06 fixture changed: before=$shaBefore after=$shaAfter"
}
if ($timedOut) {
    throw "Portable Blender $Mode exceeded $MaxProcessSeconds seconds; validation stopped after one timeout"
}
$exitCode = $process.ExitCode
if ($null -eq $exitCode) { $exitCode = 0 }
if ($exitCode -ne 0) {
    throw "PERF-P06 Blender failed for $Mode with exit code $exitCode"
}
if (-not (Test-Path -LiteralPath $Result -PathType Leaf)) {
    throw "PERF-P06 result missing: $Result"
}
...
if ($evidence.fixture_sha256_before -ne $shaBefore -or
    ...
    throw 'PERF-P06 evidence SHA mismatch'
}
if ($config.RequirePassed -and $evidence.status -ne 'passed') {
    throw "PERF-P06 $Mode evidence was not passed: $($evidence.status)"
}
...
$summary = if ($Mode -eq 'full-modal') {
    'max_tick_ms={0}; exact_slice_ms={1}; ticks={2}' -f `
        $evidence.session_report.max_tick_ms,
        $evidence.session_report.max_exact_slice_ms,
        $evidence.timer_ticks
} elseif ($Mode -eq 'dedicated') {
    'max_tick_ms={0}; exact_slice_ms={1}' -f `
        $evidence.max_tick_ms,
        $evidence.completion.result.max_exact_slice_ms
} else {
    'measured={0}; elapsed={1}' -f $evidence.measured_runs, (($evidence.measured_elapsed_ms -join '/'))
}
Write-Output ("PERF-P06 {0}: {1}; peak_ws={2}; SHA={3}" -f $Mode, $summary, $peakWorkingSet, $shaAfter)
