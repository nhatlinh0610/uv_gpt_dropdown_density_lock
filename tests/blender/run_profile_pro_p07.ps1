...
$config = @{
    'focus' = @{
        Fixture = $CcFixture
        Harness = 'profile_pro_focus_p07.py'
        Result = 'pro_07_focus.json'
    }
    'dedicated' = @{
        Fixture = $DedicatedFixture
        Harness = 'profile_pro_modal_p07.py'
        Result = 'pro_07_modal.json'
    }
    'full-modal' = @{
        Fixture = $CcFixture
        Harness = 'profile_pro_full_modal_p07.py'
        Result = 'pro_07_full_modal.json'
    }
}[$Mode]
...
foreach ($path in @($Blender, $Fixture, $Harness)) {
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "PERF-P07 required file missing: $path"
    }
}
...
if ($shaBefore -ne $shaCheck) {
    throw "PERF-P07 fixture SHA unstable before launch: $shaBefore / $shaCheck"
}
if ($Mode -in @('focus', 'full-modal') -and $shaBefore -ne $ExpectedCcSha) {
    throw "PERF-P07 cc.blend SHA mismatch: $shaBefore"
}
...
if ($shaAfter -ne $shaBefore) {
    throw "PERF-P07 fixture changed: before=$shaBefore after=$shaAfter"
}
if ($timedOut) {
...
if ($exitCode -ne 0) {
    throw "PERF-P07 Blender failed for $Mode with exit code $exitCode"
}
if (-not (Test-Path -LiteralPath $Result -PathType Leaf)) {
    throw "PERF-P07 result missing: $Result"
}
...
if ($evidence.fixture_sha256_before -ne $shaBefore -or
    ...
    throw 'PERF-P07 evidence SHA mismatch'
}
...
$summary = if ($Mode -eq 'full-modal') {
    'status={0}; ticks={1}; enum_ops={2}; records={3}; pairs={4}; max_tick_ms={5}; max_enum_slice_ms={6}' -f `
        $evidence.status,
        ...
        $evidence.session_report.max_enum_slice_ms
} elseif ($Mode -eq 'dedicated') {
    'status={0}; max_tick_ms={1}; max_enum_slice_ms={2}' -f `
        $evidence.status,
        $evidence.max_tick_ms,
        $evidence.completion.result.max_enum_slice_ms
} else {
    'measured={0}; elapsed={1}' -f $evidence.measured_runs, (($evidence.measured_elapsed_ms -join '/'))
}
Write-Output ("PERF-P07 {0}: {1}; peak_ws={2}; SHA={3}" -f $Mode, $summary, $peakWorkingSet, $shaAfter)
